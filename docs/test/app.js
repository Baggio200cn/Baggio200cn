(function () {
  ...
  const stopwords = new Set([
    ...
    "go","me","when","make","can","like","no","just","him","know","take","into","your","good","some",
  ...
  $("#loadSample").addEventListener("click", () => {
    textEl.value =
`Humans learn languages best when they practice with meaningful content.
...
online test that turns a paragraph into cloze questions. Try it now!`;
  ...
  $("#generate").addEventListener("click", () => {
    ...
    if (!raw) {
      alert("请先粘贴英文文本，或点击“加载示例文本”。");
      return;
    }
    const n = clamp(parseInt(countEl.value || "8", 10), 3, 20);
    const k = clamp(parseInt(optionsEl.value || "4", 10), 2, 6);
  ...
  $("#submit").addEventListener("click", () => {
    const questions = getRenderedQuestions();
    let correct = 0;
    questions.forEach((q) => {
  ...
  $("#showAnswers").addEventListener("click", () => {
    const questions = getRenderedQuestions();
    questions.forEach((q) => {
      const label = document.querySelector(`#${q.id} label[data-val="${q.answer}"]`);
      if (label) label.style.outline = "2px solid #16a34a";
    });
  });
  ...
  function makeClozeQuiz(raw, nQuestions, nOptions) {
    ...
    const quiz = targets.map((ans, i) => {
      ...
      return { id:`q${i+1}`, index: i+1, stem:`(${i+1}) 选择正确单词填空`, answer: ans, options };
    });
    return { quiz, preview };
  }
  ...
  function renderQuiz(quiz) {
    ...
    quiz.forEach(q => {
      ...
      block.className = "q";
      block.id = q.id;
      block.innerHTML = `<h3>${escapeHtml(q.stem)}</h3>`;
      const list = document.createElement("div");
      list.className = "choices";
      q.options.forEach((opt, j) => {
        const name = q.id;
        const id = `${q.id}_o${j+1}`;
  ...
  function getRenderedQuestions() {
    return $$(".q").map(block => {
      ...
      const userChoice = checked ? checked.value : "";
      return { id, answer, userChoice };
    });
  }
